...
//  Foodoo
//
//  Created by Techimmense Software Solutions on 20/05/24.
//

import Foundation

struct Api_FavProvider : Codable {
    let result : [Res_FavProvider]?
    let message : String?
    let status : String?
    ...
    init(from decoder: Decoder) throws {
        ...
        result = try values.decodeIfPresent([Res_FavProvider].self, forKey: .result)
        message = try values.decodeIfPresent(String.self, forKey: .message)
        status = try values.decodeIfPresent(String.self, forKey: .status)
    }
}

struct Res_FavProvider : Codable {
    let id : String?
    ...
    let cat_id : String?
    let cat_name : String?
    let item_left_quantity : String?
    let magic_food_left_quantity : String?
    ...
    enum CodingKeys: String, CodingKey {
        ...
        case item_left_quantity = "item_left_quantity"
        case magic_food_left_quantity = "magic_food_left_quantity"
    ...
    init(from decoder: Decoder) throws {
        ...
        item_left_quantity = try values.decodeIfPresent(String.self, forKey: .item_left_quantity)
        magic_food_left_quantity = try values.decodeIfPresent(String.self, forKey: .magic_food_left_quantity)
